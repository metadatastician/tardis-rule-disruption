...
cleanup() {
    echo "Received shutdown signal — stopping tardis-rule-disruption..."
...
echo "Starting tardis-rule-disruption..."
echo "  Host: ${APP_HOST:-[::]}"
...
# Examples:
#   exec /app/tardis-rule-disruption
#   exec /app/release/bin/tardis-rule-disruption start
#   exec /app/tardis-rule-disruption serve --host "${APP_HOST}" --port "${APP_PORT}"
