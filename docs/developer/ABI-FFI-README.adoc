...
# TARDIS_RULE_DISRUPTION ABI/FFI Documentation
...
│  generated/abi/tardis_rule_disruption.h                │
└─────────────────┬───────────────────────────┘
                  │
                  │ imported by
                  ▼
...
└─────────────────┬───────────────────────────┘
                  │
                  │ compiled to libtardis_rule_disruption.so/.a
                  ▼
┌─────────────────────────────────────────────┐
...
│       └── include/
│           └── tardis_rule_disruption.h   # C header (optional, can be generated)
│
├── generated/                  # Auto-generated files
│   └── abi/
│       └── tardis_rule_disruption.h       # Generated from Idris2 ABI
│
...
```bash
cd src/abi
idris2 --cg c-header Types.idr -o ../../generated/abi/tardis_rule_disruption.h
```
...
```c
#include "tardis_rule_disruption.h"

int main() {
    void* handle = tardis_rule_disruption_init();
    if (!handle) return 1;

    int result = tardis_rule_disruption_process(handle, 42);
    if (result != 0) {
        const char* err = tardis_rule_disruption_last_error();
        fprintf(stderr, "Error: %s\n", err);
    }

    tardis_rule_disruption_free(handle);
    return 0;
}
```
...
Compile with:
```bash
gcc -o example example.c -ltardis_rule_disruption -L./zig-out/lib
```

### From Idris2

```idris
import TARDIS_RULE_DISRUPTION.ABI.Foreign
...
```rust
#[link(name = "tardis_rule_disruption")]
extern "C" {
    fn tardis_rule_disruption_init() -> *mut std::ffi::c_void;
    fn tardis_rule_disruption_free(handle: *mut std::ffi::c_void);
    fn tardis_rule_disruption_process(handle: *mut std::ffi::c_void, input: u32) -> i32;
}

fn main() {
    unsafe {
        let handle = tardis_rule_disruption_init();
        assert!(!handle.is_null());

        let result = tardis_rule_disruption_process(handle, 42);
        assert_eq!(result, 0);
...
```julia
const libtardis_rule_disruption = "libtardis_rule_disruption"

function init()
    handle = ccall((:tardis_rule_disruption_init, libtardis_rule_disruption), Ptr{Cvoid}, ())
    handle == C_NULL && error("Failed to initialize")
    handle
end

function process(handle, input)
    result = ccall((:tardis_rule_disruption_process, libtardis_rule_disruption), Cint, (Ptr{Cvoid}, UInt32), handle, input)
    result
end

function cleanup(handle)
    ccall((:tardis_rule_disruption_free, libtardis_rule_disruption), Cvoid, (Ptr{Cvoid},), handle)
end
...
2. **Generate C header**
   ```bash
   idris2 --cg c-header src/interface/Abi/Types.idr -o generated/abi/tardis_rule_disruption.h
   ```
